...
import { calculateEdgesWeights, getEdgesWithIds } from '../common/utils/graph/utils.js';
import { generateGraphMaps } from '../common/utils/graph/graph-svg.js';
import { createBucket, getBucket } from '../storages/aws-s3.js';
import { getMaxFloor, getMinFloor } from '../common/utils/buildings/floors/floor.js';
import { generateColorMap, generatePOIsMaps } from '../common/utils/buildings/poi/map.js';
import { calculatePOIsCenterPoint } from '../common/utils/buildings/poi/normalize.js';


const folderPath = '../seed';
...
const baseBuildingAssetsPath = path.join(__dirname, folderPath);
...
export async function seedBuilding(buildingSeed:BuildingSeed){
    ...
    try {
        ...
        const filteredEdges = calculateEdgesWeights(nodes,edges,mapFloors,buildingSeed.map.data.zScale);
        ...
        const poisMaps = generatePOIsMaps(poisColorMap,map.data.POIs,mapFloors,map.data.mapWidth,map.data.mapHeight);
        ...
        // await createVectorTiles(mapFloorsData)
...
async function seedBuildingFiles(){
