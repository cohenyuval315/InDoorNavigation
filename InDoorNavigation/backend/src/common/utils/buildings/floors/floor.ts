...
export function getNearestFloor(altitude:number, mapFloors:MapFloor[],zScale:number) {
    try{
...
export function getCumulativeFloorHeight(floor: number, mapFloors: MapFloor[],zScale:number): number {
...
export function getFloorHeight(floor: number, mapFloors: MapFloor[],zScale:number): number {
    let cumulativeHeight = 0;
    mapFloors.forEach((mapFloor) => {
        if(mapFloor.floor < floor){
            cumulativeHeight += mapFloor.floorHeight;
