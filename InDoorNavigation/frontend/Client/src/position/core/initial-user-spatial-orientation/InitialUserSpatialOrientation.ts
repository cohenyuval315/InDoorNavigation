...
export class InitialUserSpatialOrientation { 
    ...
    async getUserPosition(
        ...
        // if(!data.wifi && data.geolocation){
        //     const position = await geoLocationMatching.getUserPosition(stateData,data.geolocation)
        //     return position
        // } else if (data.wifi && !data.geolocation){
        //     const position = await wifiFingerprint.getUserPosition(null,data.wifi);
        //     return position;
        // } else if (data.wifi && data.geolocation){
        //     if(data.geolocation.data.coords.accuracy > EngineConfiguration.initialPositionGPSAccuracyThreshold){
        //         const position = await geoLocationMatching.getUserPosition(null,data.geolocation)
        //         return position
        //     }else{
        //         const position = await wifiFingerprint.getUserPosition(null,data.wifi);
        //         return position;
