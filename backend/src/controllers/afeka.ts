...
const floorHeights = {
    0: 0, // Ground floor
    1: 3, // First floor (height in meters, for example)
...
edges.forEach(edge => {
    ...
    if (nodeA && nodeB) {
        const weight = calculateDistance(nodeA, nodeB);
        edgesWithWeights.push({ nodeA, nodeB, weight });
    }
});
...
export function generateSVGForNodesAndEdges(poiColorMap,nodes, edges, width, height,floor) {
    ...
    function getArrowPosition(x, y, direction) {
        switch (direction) {
            case Direction.UP:
    ...
    nodesOnFloor.forEach(node => {
        ...
        node.availableHeadings.forEach(direction => {
            const arrowPos = getArrowPosition(x, y, direction);
...
export function generateSVGForAreasByFloor(poiColorMap,areas, width, height, floor) {
    ...
    const areasOnFloor = areas.filter(area => area.floor === floor);

    // Create areas
    areasOnFloor.forEach(area => {
        const color = getRandomColor();
        poiColorMap[area['id']] = color;
        const points = area.mapArea.map(point => `${point.x},${point.y}`).join(' ');
        svgParts.push(`<polygon points="${points}" fill="${color}" fill-opacity="0.3" stroke="black" />`);
    });
...
const poiColorMapF_1 = {}
const svgFloorF_1 = generateSVGForAreasByFloor(poiColorMapF_1,POIs,1326,1207,-1)
const poiColorMapF0 = {}
