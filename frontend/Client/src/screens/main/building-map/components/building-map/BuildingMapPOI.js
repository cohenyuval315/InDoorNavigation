...
const BuildingPOIPoint = ({POI,rotationRef}) => {
  ...
  const [animatedRotation] = useState(new Animated.Value(rotationRef.current._value));
  // Calculate the position of the icon based on its size
  const [centerX, setCenterX] = useState(POI.mapArea[0].x);
  const [centerY, setCenterY] = useState(POI.mapArea[0].y);
...
const BuildingMapPOI = ({
    ...
    console.log("\n\nbefore:",POI.details.title, POI.mapArea)
    const POIMapArea = POI.mapArea.filter((mapCoordinates) => mapCoordinates.floor === floorIndex);
    console.log("after:", POIMapArea, "\n\n\n\n")
    const leftestDownestPoint = findLeftestDownestPoint(POIMapArea)
    
    if (!leftestDownestPoint){
        return <></>
    }
    if (!POIMapArea || POIMapArea.length == 0){
        console.error("No Map Area For POI")
        return <></>
    }

    const numOfCoordinates = POIMapArea.length;
    const renderComponent = (numOfCoordinates) => {
      switch(numOfCoordinates){
        case 0:{
          return <></>
        }
        case 1:{
          return <BuildingPOIPoint POI={POI} rotationRef={rotationRef}/>
        }
        case 2:{
          return <BuildingPOIPath POI={POI}/>
        }
        default:{
          return <BuildingPOIPolygon POI={POI}/>
        }      
      }
    };
    ...
    return (
      <MapOverlay>
        <TouchableOpacity>
          {renderComponent(numOfCoordinates)}
        </TouchableOpacity>
      </MapOverlay>
    );
